...
class OllamaModel:
    ...
    def __init__(
        ...
        # NOTE: keep embed_model_type as plain str for easier downstream use
        ...
        # Use OpenAI client as interface to call Ollama.
        # 为了防止 embedding 请求在服务器异常时长时间挂起，这里显式限制超时时间和重试次数，
        # 并允许通过环境变量进行调整：
        #   OLLAMA_TIMEOUT      – 单次请求超时（秒），默认 60
        #   OLLAMA_MAX_RETRIES  – 失败重试次数，默认 1
