...
class SciAgent_Async(BaseAgent):
    ...
    async def update_memory(
        ...
        if role == OpenAIBackendRole.SYSTEM:
            self.personality_memory.write_record(
    ...
    def init_messages(self) -> None:
        ...
        if self.orig_sys_message is not None:
            ...
            self.personality_memory.write_record(system_record)
        else:
            self.memory.clear()
            self.personality_memory.clear()
    ...
    async def step(
        ...
        if (
                ...
                and ("lama" in self.model_type.value or "qwen" in self.model_type.value or "gemma" in self.model_type.value)
                or isinstance(self.model_type, str)
                and ("lama" in self.model_type or "qwen" in self.model_type or "gemma" in self.model_type)
        ):
            ...
            while True:
                ...
                (
                    ...
                    usage_dict,
                    response_id,
                ) = await self._step_model_response(openai_messages, num_tokens)
            ...
            while True:
                ...
                tool_call_records.append(
                    await self._step_tool_call_and_update(response)
                )
            ...
            if (
                ...
                (
                    ...
                    tool_call,
                    num_tokens,
                ) = await self._structure_output_with_function(output_schema)
    ...
    async def _structure_output_with_function(
        ...
        if isinstance(response, ChatCompletion):
            tool_call_record = await self._step_tool_call_and_update(response)
        else:
            raise ValueError(
    ...
    async def _step_model_response(
            self,
