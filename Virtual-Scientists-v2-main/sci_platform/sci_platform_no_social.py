...
- Removes social interaction features (no_social version)
- Simplified team selection process without social dynamics
"""
...
from sci_team.SciTeam_no_social import Team
...
class Platform:
    ...
    async def select_single(self, agent_index):
        ...
        leader_id_int = int(scientist[9:]) 
        name = int(scientist[9:])
        arr = self.adjacency_matrix[name, :].copy()
        MIN_FINAL_TEAM_SIZE = 2 
        MAX_FINAL_TEAM_SIZE = self.max_teammember 
        min_invitees = MIN_FINAL_TEAM_SIZE - 1
        max_invitees = MAX_FINAL_TEAM_SIZE - 1

        if min_invitees >= max_invitees:
            num_to_invite = min_invitees
        else:
            num_to_invite = np.random.randint(low=min_invitees, high=max_invitees + 1)
        
        total_agents = len(arr) 
        possible_invitees_indices = list(range(total_agents))
        
        if leader_id_int in possible_invitees_indices:
            possible_invitees_indices.remove(leader_id_int)

        selected_indices = np.random.choice(
            possible_invitees_indices, 
            size=num_to_invite, 
            replace=False
        )
