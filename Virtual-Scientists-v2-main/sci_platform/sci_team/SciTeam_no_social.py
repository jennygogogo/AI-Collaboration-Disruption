...
This file is a variant of SciTeam that:
- Removes social interaction features (no_social version)
- Simplified team collaboration without social dynamics
...
class Team:
    ...
    async def select_topic(self, platform):
        ...
        if answer_msg is None:
            ...
            answer_msg.content = extract_after_think(answer_msg.content)
        ...
        if answer_pattern.search(answer_content) or len(team_memories)>=1:
            ...
            try:
                topic_msg.content = extract_after_think(topic_msg.content)
        ...
        if dialogue_msg is not None:
            dialogue_msg.content = extract_after_think(dialogue_msg.content)
    ...
    async def generate_idea(self, platform):
        ...
        try:
            ...
            selected_topics = topic if topic else "No topics."
        ...
        for turn in range(group_max_discuss_iteration):
            ...
            if x_msg is not None:
                x_msg.content = extract_after_think(x_msg.content)
