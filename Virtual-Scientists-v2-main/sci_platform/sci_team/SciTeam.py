...
class Team:
    ...
    async def group_discuss(self, platform, prompt: str = None):
        ...
        if len(platform.old_hot) > 5:
            ...
            try:
                ...
                hot_topic_reply = extract_after_think(hot_topic_reply)
        ...
        for turn in range(group_max_discuss_iteration):
            ...
            for agent in teammate:
                ...
                try:
                    ...
                    reply = reply.msg
                    reply.content = extract_after_think(reply.content)
                except:
                    reply = None
                    ...
                    if scientist_index not in self.teammate:
                        if "by the way" in reply.content or "By the way" in reply.content:
                            ...
                            try:
                                ...
                                special_guest_reply = extract_after_think(special_guest_reply)
                            except:
    ...
    async def select_topic(self, platform):
        ...
        answer = answer.msg
        answer.content = extract_after_think(answer.content)
        ...
        if answer_pattern.search(answer.content) or len(team_memories)>=1:
            ...
            topic = topic.msg
            topic.content = extract_after_think(topic.content)
        ...
        dialogue_summarization.content = extract_after_think(dialogue_summarization.content)
        ...
        for turn in range(group_max_discuss_iteration):
            ...
            for agent in teammate:
                ...
                try:
                    ...
                    reply.msg.content = extract_after_think(reply.msg.content)
        ...
        if mark_sum>=(6*platform.reviewer_num):
            ...
            try:
                ...
                if reply_msg is None:
                    ...
                    reply_msg.content = extract_after_think(reply_msg.content)
                    reply_text = reply_msg.content.lower()
                    discipline = find_best_match(filter_out_number_n_symbol(reply_text), disciplines)
            except:
                discipline = 'computer science'
            try:
                ...
                if reply_msg is None:
                    ...
                    reply_msg.content = extract_after_think(reply_msg.content)
                    keywords = extract_between_json_tags(reply_msg.content)
            except:
                keywords = 'None'
            file_dict={}
            ...
            if len(self.mark_history)>=2:
                ...
                reply = await self._safe_step(
                    teammate[0],
                    format_failure_check_prompt,
                    timeout=6000.0,
                    context="failure_check",
                )
                if reply is None:
                    self.state = 5
                    return
